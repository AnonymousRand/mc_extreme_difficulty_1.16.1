    ...
    public void tick() {
        ...
        if (this.teleportToPlayer > 300) { //has a 1% chance every tick to teleport to within follow_range-2 to follow_range+5 blocks of nearest player if it has not seen a player target within follow range for 15 seconds
            if (rand.nextDouble() < 0.01) {
                ...
                if (player != null) {
                    BlockPosition pos = coordsFromHypotenuse.RandomCoordsFromHypotenuseAndAngle(new BlockPosition(player.locX(), player.locY(), player.locZ()), hypo, this.locY() + 2.0, 361.0); //gets coords for a random angle (0-360) with fixed hypotenuse to teleport to (so possible teleport area is a washer-like disc around the player)
