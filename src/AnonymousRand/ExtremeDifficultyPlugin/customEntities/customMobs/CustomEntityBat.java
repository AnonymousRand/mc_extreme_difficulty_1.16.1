...
public class CustomEntityBat extends EntityBat {
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        ...
        if (damagesource.getEntity() instanceof EntityPlayer && this.getHealth() - f > 0.0 && this.firstDuplicate) { /**summons 15-20 bats when hit by player and not killed for the first time (also 2 aggressive bats after 45 attacks)*/
            ...
            for (int i = 0; i < this.random.nextInt(6) + 15; i++) {
            ...
            if (this.attacks >= 45) {
                CustomEntityBat newCustomBat;

                for (int i = 0; i < 2; i++) {
                    newCustomBat = new CustomEntityBat(this.getWorld());
                    newCustomBat.setPosition(this.locX(), this.locY(), this.locZ());
                    this.getWorld().addEntity(newCustomBat, CreatureSpawnEvent.SpawnReason.NATURAL);
                }
            }
        }
