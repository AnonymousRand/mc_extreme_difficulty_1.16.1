    ...
    public void run() {
        ...
        switch (type) {
            case 1 -> { //exploding arrows
                for (int i = 0; i < 4; i++) {
                    pos = coordsFromHypotenuse.CoordsFromHypotenuseAndAngle(this.bukkitWorld, new BlockPosition(playerLoc.getX(), playerLoc.getY(), playerLoc.getZ()), this.random.nextDouble() * this.maxRadius, playerLoc.getY() + this.random.nextDouble() * 20.0 + 20.0, 361.0); //gets coords for a random angle (0-360) with fixed hypotenuse to summon projectile at

                    this.explodingArrow = new CustomEntityArrowExploding(this.nmsWorld, this.vec, (byte)(this.random.nextDouble() < 0.25 ? 1 : 0), this.bukkitShooter, 1.0F); /**25% of arrows are piecing 1*/
            ...
            case 2 -> { //fireballs
                for (int i = 0; i < 4; i++) {
                    pos = coordsFromHypotenuse.CoordsFromHypotenuseAndAngle(this.bukkitWorld, new BlockPosition(playerLoc.getX(), playerLoc.getY(), playerLoc.getZ()), this.random.nextDouble() * this.maxRadius, playerLoc.getY() + this.random.nextDouble() * 40.0 + 15.0, 361.0);
            ...
            case 3 -> { //lightning
                pos = coordsFromHypotenuse.CoordsFromHypotenuseAndAngle(this.bukkitWorld, new BlockPosition(this.playerLoc.getX(), this.playerLoc.getY(), this.playerLoc.getZ()), this.random.nextDouble() * this.maxRadius, this.bukkitWorld.getHighestBlockYAt(this.playerLoc), -361.0);
