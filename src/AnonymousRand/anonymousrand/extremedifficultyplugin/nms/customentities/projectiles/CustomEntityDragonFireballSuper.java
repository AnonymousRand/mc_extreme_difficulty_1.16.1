...
public class CustomEntityDragonFireballSuper extends EntityDragonFireball {
    ...
    protected void a(MovingObjectPosition movingObjectPosition) {
        ...
        for (int i = -3; i < 3; i++) { /* area effect clouds are 6 blocks high, 9 blocks wide and take 5 ticks less to start doing damage, but only last 10 seconds */
            ...
            for (EntityLiving entity : entities) {
                if (NMSUtil.distSq(this, entity, true) < 144.0D) { /* super fireball area effect clouds snap on to location of nearest player within 12 blocks horizontally */
    ...
    public double d(Vec3D vec3d) {
        double d0 = this.locX() - vec3d.x; /* for determining distance to entities, y-level does not matter, e.g. mob follow range, attack (can hit player no matter the y-level) */
