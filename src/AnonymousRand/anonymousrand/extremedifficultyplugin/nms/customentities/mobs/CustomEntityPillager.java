...
import AnonymousRand.anonymousrand.extremedifficultyplugin.nms.customgoals.attackmvmt.CustomPathfinderGoalAttackMvmtRangedHandheld;
...
public class CustomEntityPillager extends EntityPillager implements ICustomHostile, IGoalRemovingMob {
    ...
    public void initPathfinder() {
        ...
        this.goalSelector.a(2, new CustomPathfinderGoalAttackRangedCrossbow<>(this, 16)); /* shoots every 16 ticks; uses the custom goal that attacks regardless of the y-level (the old goal stopped the mob from attack even if the mob has already recognized a target via CustomNearestAttackableTarget goal) */
        this.goalSelector.a(2, new CustomPathfinderGoalAttackMvmtRangedHandheld<>(this, Items.CROSSBOW));
    ...
    public void a(EntityLiving attackTarget, float distFactor) { // shoot()
        ...
        new RunnableMobShootArrows(this, attackTarget, 1, this.attackNum % 18 == 0 ? 6 : 1, 3.0, random.nextDouble() < (this.attackNum % 24 == 0 ? 1.0 : 0.075) ? 1 : 0, false, true).run(); /* shoots a knockback arrow every 18th attack; 7.5% of arrows shot are piercing 1 (100% for knockback arrow); arrows do not lose y-level */
    }
