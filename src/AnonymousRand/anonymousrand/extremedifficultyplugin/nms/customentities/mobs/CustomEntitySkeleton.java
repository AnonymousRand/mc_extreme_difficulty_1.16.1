...
public class CustomEntitySkeleton extends EntitySkeleton implements ICustomHostile, IAttackLevelingMob {
    ...
    protected void initPathfinder() { /* no longer avoids sun and wolves or targets iron golems */
        ...
        this.goalSelector.a(4, new CustomPathfinderGoalAttackRangedSkeleton<>(this, 25)); /* skeletons shoot 25% slower; uses the custom goal that attacks regardless of the y-level (the old goal stopped the mob from attack even if the mob has already recognized a target via CustomNearestAttackableTarget goal) */
