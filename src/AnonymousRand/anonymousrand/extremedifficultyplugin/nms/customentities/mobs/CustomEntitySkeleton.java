...
public class CustomEntitySkeleton extends EntitySkeleton implements ICustomHostile, IAttackLevelingMob {
    ...
    protected void initPathfinder() { /* no longer avoids sun and wolves or targets iron golems */
        ...
        this.goalSelector.a(4, new CustomPathfinderGoalRangedBowAttack<>(this, 1.0, 25, 32.0F)); /* skeletons shoot 25% slower; uses the custom goal that attacks regardless of the y-level (the old goal stopped the mob from attacking even if the mob has already recognized a target via CustomNearestAttackableTarget goal) */
    ...
    public void tick() {
        ...
        if (this.ticksLived % (random.nextInt(2) + 2) == 0) {
            if (this.getGoalTarget() != null) {
                ...
                if (NMSUtil.distSq(this, target, false) > Math.pow(this.getDetectionRange(), 2)) { // deaggro if player out of y-level-included sphere for performance reasons // todo if not revamping y-level attacking globally: how about just not ignore y level and use vanilla nearestattackabletarget?
