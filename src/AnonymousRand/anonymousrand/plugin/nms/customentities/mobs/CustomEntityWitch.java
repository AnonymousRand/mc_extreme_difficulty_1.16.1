...
import AnonymousRand.anonymousrand.plugin.util.bukkitrunnables.RunnableRingOfFireballs;
...
public class CustomEntityWitch extends EntityWitch implements ICustomHostile, IGoalRemovingMob {
    ...
    public void initPathfinder() {
        ...
        this.goalSelector.a(1, new CustomPathfinderGoalAttackRanged<>(this, 5)); /* throws a potion every 5 ticks and uses the custom goal that attacks regardless of the y-level (the old goal stopped the mob from attack even if the mob has already recognized a target via CustomNearestAttackableTarget goal) */
        this.goalSelector.a(1, new CustomPathfinderGoalAttackMvmtRanged<>(this));
    ...
    public void a(EntityLiving attackTarget, float distFactor) { // shoot()
        ...
        if (!this.m()) {
            Vec3D vec3d = attackTarget.getMot();
            double d0 = attackTarget.locX() + vec3d.x - this.locX();
            double d1 = attackTarget.getHeadY() - 1.1 - this.locY();
            double d2 = attackTarget.locZ() + vec3d.z - this.locZ();
            ...
            if (attackTarget instanceof EntityRaider) {
                if (attackTarget.getHealth() <= 10.0F) { /* gives fellow raiders instant health 2 instead of instant health 1 below 5 hearts */
            ...
            } else if (f1 >= 6.0F && !attackTarget.hasEffect(MobEffects.SLOWER_MOVEMENT)) { /* gives slowness 2 up to 6 blocks away */
                potionregistry = Potions.STRONG_SLOWNESS;
            } else if (f1 < 6.0F && !attackTarget.hasEffect(MobEffects.WEAKNESS)) { /* 100% to give weakness when player within 6 blocks */
...
//    }
    ...
    public void tick() {
        ...
        if (this.attacks == 30 && !this.a30) { /* after 30 attacks, witches summon area effect clouds wherever it goes */
            this.a30 = true;
            this.goalSelector.a(1, new CustomPathfinderGoalSpawnBlocksEntitiesOnMob(this, this.newAEC, 1, 0, 1, 0, 1.0));
        }
    }
