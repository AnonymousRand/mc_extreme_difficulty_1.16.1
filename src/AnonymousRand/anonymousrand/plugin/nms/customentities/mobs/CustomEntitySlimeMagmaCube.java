...
import AnonymousRand.anonymousrand.plugin.nms.customgoals.CustomPathfinderGoalBreakBlocksAround;
import AnonymousRand.anonymousrand.plugin.nms.customgoals.CustomPathfinderGoalAttackMeleeSlime;
import AnonymousRand.anonymousrand.plugin.util.bukkitrunnables.RunnableConstantlySpawnBlocksEntities;
...
public class CustomEntitySlimeMagmaCube extends EntityMagmaCube implements ICustomHostile, IAttackLevelingMob, IGoalRemovingMob {
    ...
    protected void initPathfinder() { /* no longer targets iron golems */
        ...
        this.goalSelector.a(1, new CustomPathfinderGoalAttackMeleeSlime<>(this)); /* uses the custom goal that atstacks regardless of the y-level (the old goal stopped the mob from attack even if the mob has already recognized a target via CustomNearestAttackableTarget goal) */
        this.goalSelector.a(1, new CustomPathfinderGoalAttackMvmtMelee<>(this)); // todo make sure works
...
//    }
    ...
    public void tick() {
        ...
        if (this.ticksLived == 5) {
            if (this.getSize() > 3) {
                this.goalSelector.a(0, new CustomPathfinderGoalBreakBlocksAround(this, 10, this.getSize() / 4 + 1, this.getSize() / 4 + 1, this.getSize() / 4 + 1, this.getSize() / 4 + 1, false)); /* Breaks most blocks around the mob periodically */
            }
        }
    }
